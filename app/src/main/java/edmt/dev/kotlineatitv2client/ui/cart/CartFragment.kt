package edmt.dev.kotlineatitv2client.ui.cart
...
import edmt.dev.kotlineatitv2client.Adapter.MyCartAdapter
import edmt.dev.kotlineatitv2client.Callback.ILoadTimeFromFirebaseCallback
import edmt.dev.kotlineatitv2client.Callback.IMyButtonCallback
import edmt.dev.kotlineatitv2client.Common.Common
import edmt.dev.kotlineatitv2client.Common.MySwipeHelper
import edmt.dev.kotlineatitv2client.Database.CartDataSource
import edmt.dev.kotlineatitv2client.Database.CartDatabase
import edmt.dev.kotlineatitv2client.Database.CartItem
import edmt.dev.kotlineatitv2client.Database.LocalCartDataSource
import edmt.dev.kotlineatitv2client.EventBus.CountCartEvent
import edmt.dev.kotlineatitv2client.EventBus.HideFABCart
import edmt.dev.kotlineatitv2client.EventBus.UpdateItemInCart
import edmt.dev.kotlineatitv2client.Model.Order
import edmt.dev.kotlineatitv2client.R
import edmt.dev.kotlineatitv2client.Remote.ICloudFunctions
import edmt.dev.kotlineatitv2client.Remote.RetrofitCloudClient
...
class CartFragment : Fragment(), ILoadTimeFromFirebaseCallback {
    ...
    override fun onResume() {
        super.onResume()
        calculateTotalPrice()
        if (fusedLocationprovideCLient != null)
    ...
    private fun initLocation() {
        ...
        fusedLocationprovideCLient = LocationServices.getFusedLocationProviderClient(context!!)
        fusedLocationprovideCLient!!.requestLocationUpdates(
            locationRequest, locationCallback,
            Looper.getMainLooper()
        )
    ...
    private fun initViews(root: View) {
        ...
        btn_place_order.setOnClickListener {
            ...
            rdi_other_address.setOnCheckedChangeListener { compoundButton, b ->
                if (b) {
                    edt_address.setText("")
                    edt_address.setHint("Masukkan Alamat Anda")
                    txt_address.visibility = View.GONE
                }
            }
            rdi_ship_to_this_address.setOnCheckedChangeListener { compoundButton, b ->
                if (b) {
                    fusedLocationprovideCLient!!.lastLocation
                                    ...
                                    .subscribe({ cartitems: List<CartItem>? ->
                                        ...
                                        compositeDisposable.add(cloudFunctions.submitPayment(
                                                ...
                                                .subscribe({ braintreeTransaction ->
                                                    ...
                                                    if (braintreeTransaction.succes)
                                                    {
                                                        //Create Order
                                                        val finalPrice = totalPrice
                                                        val order = Order()
